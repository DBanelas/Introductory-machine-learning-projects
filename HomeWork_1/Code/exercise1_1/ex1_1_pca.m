...
plot(X(:,1), X(:,2) , 'ro');
title('Data with sample \mu and \sigma');
...
%added code, will be needed for demonstration purposes
%need to comment again in order for the program to work
%  figure(50);
%  plot(X_norm(:,1), X_norm(:,2) , 'bo');
%  title('Normalized data with \mu = 0 and \sigma = 1');
%  fprintf('Program paused. Press enter to continue.\n');
%  pause;
...
% total variance from sum of eigenvalues
totalVar = sum(sum(S));
%largest eigenvalue is at S(1,1)
PCvariance = [S(1,1)/totalVar; S(2,2)/totalVar];
...
hold off
title('Normalized data projected on the first PC')
fprintf('Program paused. Press enter to continue.\n');
pause;
...
%Variables needed in order to calculate the percentage of the total
%variance in the first K principal components.
eigVals = sum(S);
totalVar = sum(eigVals);

fprintf('Total variance in K = %d principal components is %f / 100\n', K , (sum(eigVals(1:K))/totalVar)*100);
